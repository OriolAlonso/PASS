...
function appendNews(news, seccio, isNew) {
    var articleHTML = "<article class='col-lg-6 col-md-12 col-sm-12 col-12 activitat-hora'>" +
        "<h2>" + news.titol + "</h2>" +
        "<p>" + news.text + "</p>" +
        "<p>Data i hora: " + new Date(news.data).toLocaleString() + "</p>";

    if (isNew) {
        articleHTML += "<button class='btn btn-warning modificar' data-seccio='" + seccio + "'>Modificar</button>";
    }

    articleHTML += "<i class='fas fa-trash eliminar' title='Esborra notícia'></i>" +
        "</article>";

    $("#" + seccio + "-noticies").append(articleHTML);
    // Oculta el mensaje "Sense notícies" cuando se añade una noticia
    $("#no" + seccio.charAt(0).toUpperCase() + seccio.slice(1) + "Noticies").hide();
}
...
function creaNoticia(formId) {
    var seccio = $("#" + formId + " #seccio" + (formId === "formulariModificar" ? "Modificar" : "Afegir")).val();
    var titol = $("#" + formId + " #titol" + (formId === "formulariModificar" ? "Modificar" : "Afegir")).val();
    ...
    if (seccio && titol) {
        ...
        if (seccio === "secundari") {
            news.text = $("#" + formId + " #textNoticia" + (formId === "formulariModificar" ? "Modificar" : "Afegir")).val();
        }
